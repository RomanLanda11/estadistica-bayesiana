...
r2<-ggplot(ganancias2_df) +
      ...
      geom_density(color = "blue", size = 1)
r2
mean(ganancias2_df$ganancias)
```

Se grafica la distribución aproximada de la ganancia anual. En este se diferencian tres modas, estas corresponden a la cantidad de años en los cuales se usaron mayormente las máquinas uno, tres y 2dos respectivamente.
...
r3<-ggplot(ganancia3_df) +
  ...
  labs(x="Ganancias anuales",y="")
r3
mean(mean(ganancia3_df$wins))
```


Se grafica la distribución aproximada de la ganancia anual. En este se diferencian dos modas, las cuales corresponden a la cantidad de años en los cuales se usaron mayormente las máquinas tres y dos respectivamente.Con esta estrategia. A diferencia de la anterior, no se obtiene la moda que representa a la máquina 1, se supone que esto sucede porque la estrategia identifica en casi todas las simulaciones que no es la mejor.
La media de las ganancias anuales es 187.3 unidades monetarias.
Además se puede notar que mayormente se obtuvieron ganancias altas, por lo que suponemos que se utilizó en mayor medida la máquina dos.

El método es bayesiano ya que utiliza la máquina con mayor esperanza de éxito y estas se actualizan en base a los resultados obtenidos tirada tras tirada.


```{r}
mean(ganancia3_df$wins)
```
...
#Gráficos
graficar(cont_tiradas, cont_exitos, wins, alphas, betas)
print(paste("Las ganancias totales en ese año resultan ",wins[366]))
```
...
```{r}
quantile(ganancia5_df$wins,0.05)
mean(ganancia5_df$wins)
```
...
La ganancia media anual resulta igual a 185.069 unidades monetarias.
...
Por último se regristra la evolución diaria de las ganancias, obteniendo una ganancia total igual a 206 unidades monetarias.
...
Mediante el gráfico de las ganancias anuales se observa una distribución unimodal, con una leve asimetría a la izquierda en los cuales la ganancia anual se encuentra entre 165.95 y 212 unidades monetarias un 90% de las veces.La ganancia anual mediana resulta igual a 193 unidades monetarias

```{r}
quantile(ganancia6_df$wins, c(0.05,0.95))
mean(ganancia6_df$wins)
median(ganancia6_df$wins)
```
...
La media de las ganancias anuales es 188.108 unidades monetarias.

```{r}
mean(ganancia7_df$wins)
```
...
Se presenta una tabla con 

```{r}
df_ganancias <- data.frame(Estrategia = rep(0:7,each=1000), Ganancia = c(ganancias0_df$ganancias, ganancias1_df$ganancias, ganancias2_df$ganancias, ganancia3_df$wins, ganancia4_df$wins, ganancia5_df$wins, ganancia6_df$wins, ganancia7_df$wins))
...
# Renombrar columnas
colnames(tabla_combinada) <- c("Estrategia", "Media", "Desvio", "Mediana")
