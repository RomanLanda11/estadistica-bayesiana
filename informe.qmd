...
r0<-ggplot(ganacias_df) +
      aes(x = ganacias) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)
r0
```
Sin dudas esta es la mejor situación posible, se visualiza una ganacia anual con una distribución que luce normal, centrada 200 unidades monetarias, pero recordemos, realmente no se conoces cual es la mejor máquina, asique, a explorar y/o explotar...
...
r2<-ggplot(ganacias_df) +
      aes(x = ganacias) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)
r2
mean(ganacias_df$wins)
```
...
r3<-ggplot(ganancia_df) +
      aes(x = wins) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)
r3
mean(mean(ganancia_df$wins))
```
...
Luego se repite este procedimiento para simular 1000 años jugados con esta estrategia, a su vez, se varia el timepo de exploración elegido ($\epsilon$) y se analizan los resultados.
