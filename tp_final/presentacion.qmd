...
library(bayesplot)
mcmc_intervals(stan_model, pars = c("a[1]", "a[2]"))
...
mcmc_intervals(stan_model, pars = c("a","b1", "b2", "b4"))
...
df_draws_long <- tidyr::pivot_longer(df_draws, c("a", "b1", "b2", "b4"), names_to = "param") #pivoteo la tabla
head(df_draws_long)
