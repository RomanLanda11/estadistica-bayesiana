...
dist_edad+edad_diag/hipo_diag&plot_annotation(caption = "Variables demográficas")&
...
#  guides(color = guide_legend(title = "Sexo"))+
  # scale_color_manual(name = "Sexo",labels = c("Mujer", "Hombre"), values = c("Mujer" = "#F57C73", "Hombre" = "#00BEC3"))

g2 <- ggplot(df)+
  aes(x=edad, y=lh_cortex_superiorfrontal_thickness,color=sexo)+
  geom_point()+
  geom_smooth(method = "lm", se=FALSE, color = "#027368")+
  labs(x = "", y = "Espesor corteza sup. frontal", color = "Sexo")+
  scale_color_discrete(labels = c("Mujer", "Hombre"))+
...
g4 <- ggplot(df)+
  ...
  geom_point()+
  geom_smooth(method = "lm", se=FALSE, color = "#027368")+
  labs(x = "Edad", y = "Vol. Intracraneal", color = "Sexo")+
  scale_color_discrete(labels = c("Mujer", "Hombre"))+
theme(legend.position = "bottom")
...
(g1 | g2) /
(g3 | g4) +plot_annotation(caption = "Gráfico de Edad vs variables morfológicas según el sexo de la persona")&
  theme(plot.caption = element_text(hjust = 0),
...
(campo_res+campo_diag)/(sex_diag+res_diag)/plot_annotation(caption = "Exploratorio categoricas")&
  theme(plot.caption = element_text(hjust = 0))
```
...
#load("")
loo_1 <- loo(stan_model, pars = "log_likelihood")
loo_1
#load("")
...
fila1 <- as.data.frame(loo_1$estimates)[1, ]
names(fila1) <- c("Estimación", "Desvío Estándar")
row.names(fila1) <- "elpd loo"
kable(fila1) %>% 
row_spec(0, background = "#53A6A6") %>% 
kable_styling
...
TODO: RO para comparacion de grupos y graficos de ppcheck
=======
...
```
>>>>>>> 654ef94b6b90aaca346b5843bd950551e74c2603

```{r}
# y_rep_1 <- extract(stan_model, "y_rep")$y_rep
# ppc_dens_overlay(df$edad, y_rep_1[sample(2000, 200), ]) +
...
$$Enfermedad_i | \pi_i \sim Bernoulli(\pi_i) $$ $$logit(\pi_i) = \beta_0 Intensidad1.5 + \beta_1 . Intensidad3_i + \beta_2 . Phillips_i + \beta_3 . Siemens_i$$
...
if(correr_modelos){
  ...
  save(stan_model, file="modelo_tecnologicas_run.stan")
}
<<<<<<< HEAD
load("models/modelo_tecnologicas_run.stan")
=======
...
# tiene sentido hacerlo?
<<<<<<< HEAD
#TODO ajustar solo marca y solo intensidad y comparar
oad("models/modelo_tecnologicas_run.stan") # solo marca
=======
...
comp_tec <- loo_compare(loo_1tec,loo_2tec,loo_3tec)

fila1 <- round(as.data.frame(loo_1tec$estimates)[1, ],4)
fila2 <- round(as.data.frame(loo_2tec$estimates)[1, ],4)
fila3 <- round(as.data.frame(loo_3tec$estimates)[1, ],4)
tabla1_tec <- rbind(fila1[1], fila2[1], fila3[1])

columnas <- round(as.data.frame(comp_tec)[,1:2],4)
li <- c(0, -0.382494-1.96*1.9635, -7.045310-1.96*7.0468)
ls <- c(0, -0.382494+1.96*1.9635, -7.045310+1.96*7.0468)

ic <- c("[0.0000;0.0000]", "[-4.2309;3.4659]", "[-20.8570;6.7664]")

tabla1_tec <- cbind(tabla1_tec, columnas,ic)

names(tabla1_tec) <- c("Estimación", "Diferencia elpd", "Desvío Estándar de la diferencia", "IC para la diferencia")
row.names(tabla1_tec) <- c("elpd loo modelo 1", "elpd loo modelo 2","elpd loo modelo 3") 
...
df_draws <- as.data.frame(extract(stan_model)) # tomo las muestras en un df
df_draws_long <- tidyr::pivot_longer(df_draws, c("a.1", "a.2", "a.3"), names_to = "param") #pivoteo la tabla
...
```{r}
#mean(df_draws$a < 0)
eta_ge<- df_draws$a.1 # + df_draws$a.1 +df_draws$a.3
eta_phi<- df_draws$a.2 
eta_siem<- df_draws$a.3 
pi_ge <- expit(eta_ge)
pi_phi <- expit(eta_phi)
pi_siem <- expit(eta_siem)
p_ge <- data.frame(x = pi_morf) |> 
  ggplot() + 
  geom_histogram(aes(x = x), fill = "darkcyan")+#, bins = 30) +
<<<<<<< HEAD
  labs(x = "P(Enfermo | General Electric)", y = NULL) +
=======
...
p_phi <- data.frame(x = pi_phi) |> 
  ggplot() + 
  geom_histogram(aes(x = x), fill = "darkcyan")+#, bins = 30) +
<<<<<<< HEAD
  labs(x = "P(Enfermo | Phillips)", y = NULL) +
=======
...
p_siem <- data.frame(x = pi_siem) |> 
  ggplot() + 
  geom_histogram(aes(x = x), fill = "darkcyan")+#, bins = 30) +
  labs(x = "P(Enfermo | Siemens)", y = NULL) +
  theme_bw() +
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())

p_ge/p_phi/p_siem/plot_annotation(caption = "")&
  theme(plot.caption = element_text(hjust = 0))
...
loo_2morf
comp_morf <- loo_compare(loo_1morf,loo_2morf)

fila1 <- round(as.data.frame(loo_1morf$estimates)[1, ],4)
fila2 <- round(as.data.frame(loo_2morf$estimates)[1, ],4)
tabla1_morf <- rbind(fila1[1], fila2[1])

columnas <- round(as.data.frame(comp_morf)[,1:2],4)
ic <- c("[0.0000;0.0000]", "[-8.7030;2.2134]")

tabla1_morf <- cbind(tabla1_morf, columnas,ic)

names(tabla1_morf) <- c("Estimación", "Diferencia elpd", "Desvío Estándar de la diferencia", "IC para la diferencia")
row.names(tabla1_morf) <- c("elpd loo modelo 1", "elpd loo modelo 2") 
...
n_seq <- 100
n_draws <- 18000
edad_seq <- seq(30, 95, length.out = n_seq)

eta_matrix <- apply(df_draws[c("a", "b1", "b4")],  1, calcular_eta, x = edad_seq)
#dim(eta_matrix)
