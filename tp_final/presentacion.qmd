...
$$logit(\pi_i) = \beta_0 + \beta_1 . Edad_i + \beta_2 . Sexo_i$$ \### Variables tecnologicas:

$$Enfermedad_i | \pi_i \sim Bernoulli(\pi_i) $$ $$logit(\pi_i) = \beta_0 + \beta_1 . Intensidad_i + \beta_2 . Phillips_i + \beta_3 . Siemens_i$$
...
if(correr_modelos){
  ...
  save(stan_model, file="modelo_demograficas.stan")
}
load("modelo_demograficas.stan")
stan_model
...
pairs(stan_model)
# Resumen del modelo
print(stan_model, pars = c("a", "b"))

library(bayesplot)
mcmc_intervals(stan_model, pars = c("a[1]", "a[2]", "b[1]", "b[2]"))
```
...
if(correr_modelos){
  ...
  save(stan_model, file="modelo_tecnologicas.stan")
}
load("modelo_tecnologicas.stan")
stan_model
...
los ic de marca se solapan y cubren al 0, marca e intensidad no signif? pensarlo
...
if(correr_modelos){
  ...
  save(stan_model, file="modelo_morfologicas.stan")
}
load("modelo_morfologicas.stan")
stan_model
...
mcmc_intervals(stan_model, pars = c("a", "b[1]", "b[2]", "b[3]", "b[4]"))
