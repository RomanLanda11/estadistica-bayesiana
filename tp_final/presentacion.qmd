...
load("modelo_demograficas_run.stan")
stan_model
...
load("modelo_tecnologicas_run.stan")
stan_model
...
load("modelo_morfologicas_a14.stan")
stan_model
...
loo_1 <- loo(stan_model, pars = "log_likelihood")
loo_1
load("modelo_morfologicas_a124yrep.stan")
loo_2 <- loo(stan_model, pars = "log_likelihood")
loo_2
loo_compare(loo_1,loo_2)
```
la dif entre modelos no es significativa, me quedo ocn el mas parsimoñoso
...
pi_morf <- expit(eta_morf)
data.frame(x = pi_morf) |> 
  ggplot() + 
  geom_histogram(aes(x = x), fill = "darkcyan")+#, bins = 30) +
  labs(x = "P(Enfermo | hipocampo y sup_frontal medias)", y = NULL) +
  theme_bw() +
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())
