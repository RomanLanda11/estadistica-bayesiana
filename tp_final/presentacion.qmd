...
$$Enfermedad_i | \pi_i \sim Bernoulli(\pi_i) $$
$$logit(\pi_i) = \beta_0 + \beta_1 . Intensidad_i + \beta_2 . Phillips_i + \beta_3 . Siemens_i$$
...
if(correr_modelos){
  ...
  stan_data <- list(
    N = nrow(df),
    hipocampo = df$lh_subcx_hippocampus_volume ,       
    intercraneal = df$xh_general_etiv_volume,
    fusiforme = df$lh_cortex_fusiform_volume,
    sup_frontal = df$lh_cortex_superiorfrontal_thickness,
    y = df$enfermedad
    )
