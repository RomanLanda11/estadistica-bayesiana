...
```{r}
pi <- matrix( nrow = 200,ncol = 128)
for (j in 1:200) {
  pi[j,] <- rnorm(128,0,1.5)
}

pi <- expit(pi)
#y_rep_1 <- extract(stan_model, "y_rep")$y_rep
#y_rep_1[sample(2000, 200), ]
bernoulli_matrix <- matrix(rbinom(200 * 128, size = 1, prob = c(pi)), nrow = 200, ncol = 128)


ppc_bars(df$enfermedad,bernoulli_matrix)
```
...
pi <- matrix( nrow = 200,ncol = 128)
for (j in 1:200) {
  pi[j,] <- rnorm(128,0,2)+ rnorm(128,0,1)*df$hipocampo_norm  + rnorm(128,0,1)*df$sup_frontal_norm
}
...
quantile(pi_morf_bajos, 0.025)
quantile(pi_morf_bajos, 0.975)

pi_morf_altos <- expit(eta_morf_altos)
quantile(pi_morf_altos, 0.025)
quantile(pi_morf_altos, 0.975)

p_bajos <- data.frame(x = pi_morf_bajos) |> 
  ggplot() + 
  geom_histogram(aes(x = x), fill = "darkcyan")+#, bins = 30) +
  labs(x = "P(Enfermo | valores chicos de vol. del hipocampo y corteza sup. frontal)", y = NULL) +
  scale_x_continuous(limits = c(0,1))+
  theme_bw() +
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())
...
p_altos <- data.frame(x = pi_morf_altos) |> 
  ggplot() + 
  geom_histogram(aes(x = x), fill = "darkcyan")+#, bins = 30) +
  labs(x = "P(Enfermo | valores altos del vol. del hipocampo y corteza sup. frontal)", y = NULL) +
  scale_x_continuous(limits = c(0,1))+
  theme_bw() +
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())
...
if(correr_modelos){
  ...
  stan_model <- stan( 
    file = "models/modelo_combinado.stan", 
    data = stan_data,
    chains = 4,           # Cantidad de cadenas
    warmup = 500,         # Iteraciones de warmup
    iter = 5000,          # Iteraciones totales (warmup + sampleo)
  ...
  save(stan_model, file="models/modelo_combinado_rnu.stan")
}


mcmc_intervals(stan_model, pars = c("a","e1", "e2","e3", "e4", "m1", "m4", "t[1]", "t[2]"))


load("models/modelo_combinado_rnu.stan")
loo_1comb<- loo(stan_model, pars = "log_likelihood")

loo_compare(loo_1morf, loo_1comb)
```
