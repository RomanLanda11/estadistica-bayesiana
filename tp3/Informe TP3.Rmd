...
Sabemos que la temperatura del cuerpo de Segio satisface la siguiente
...
g1 <- ggplot(data = df) +
  ...
  scale_color_manual(
    values = c("#982597","#663364", "#897289", "purple", "#7e0b7d", "#af95d2", "#5f3ae0")
  ) +
...
En términos generales, el cadáver pierde $2 °C$ de temperatura durante
la primera hora después del deceso. Luego pierde alrededor de $1.5 °C$
por hora durante las siguientes 12 horas. En las horas posteriores, la
temperatura desacelera su descenso hasta llegar a la temperatura
ambiente, aproximadamente 24 hs luego de la muerte.
...
muerte es $e^{2.639}°C=14°C$. La pendiente es $-r=-0.13$, esto significa
que a medida que aumenta una hora, el logaritmo de la diferencia entre
la temperatura corporal y la temperatura ambiente disminuye en $0.1°C$.

Para resolver nuestro objetivo proponemos ajustar un modelo lineal
normal, en el que tomaremos como variable explicativa el tiempo. A
continuación se presentan los hechos transcurridos en la noche del
fallecimiento de Sergio para ingresar estos datos en el modelo.
...
el descenso de temperatura post mortem ( **algor mortis** ) es aquella
que presentamos con $r=0.15$. Luego, tomamos las medidciones de las 6:45
am y 8:15 am, las proyectamos a traves de nuestra función y obsevarmos
...
Viendo esto sabemos que a las 6:45 am el cuerpo deberia llevar 2.19
horas sin vida y a las 8:45 am 3,786 horas (lo que equivale a 2 hs y 12
minutos, y 3 hs y 47 minutos respectivamente). Por lo que pensamos que
la hora de muerte puede estas alrededor de las 4:30 am.
...
vida y la temperatura ambiente irá disminuyendo conforme pasen las
horas. Por lo tanto, asumimos un prior de $\beta_1$ con distribución
Normal con media $\mu = -0.15$ y desvío $\theta_1 = 0.04$.
...
tdt_graf <- ggplot(df_aux)+
  ...
  geom_hline(yintercept = 37.5, linetype = "dashed", color = "#7e0b7d")+
  geom_hline(yintercept = 36.5, linetype = "dashed", color = "#7e0b7d")+
  theme_few()
#tdt_graf
...
grid.arrange(g, tdt_graf, ncol = 2,
             botton = textGrob("Gráfico 4: Priors",
                            x = 1,
                            just = "center"))
```
...
través de muestras. Para realizar esto se toman valores puntuales de t.
...
ggplot(df_muestras, aes(x = y_pred)) +
  ...
  labs(
    ...
    caption = "Gráfico 5: Histograma de y_pred para t = 0, 6, 12 y 24"
  ) +
...
#  labs(caption = "Gráfico 6: trace plot de las muestras") +
#    theme(plot.caption = element_text(hjust = 0.5)))
...
tdt_graf <- ggplot(df_aux)+
  aes(x = tiempo, y = tdt, group = curva)+
  geom_rect(aes(ymin=36.5, ymax=37.5, xmin=-1, xmax=24),fill = "purple", alpha=0.01)+
  geom_line(alpha = 0.1, color = "gray2")+
  theme(legend.position = "none")+
...
grid.arrange(posterior1, tdt_graf, ncol=2,
             botton = textGrob("Gráfico 7: Posteriors de un dato",
                            x = 1,
                            just = "center"))
```
...
grid.arrange(posterior2,tdt_graf, ncol=2,
             botton = textGrob("Gráfico ?: Posteriors para dos datos",
                            x = 1,
                            just = "center"))
```
...
grid.arrange(posterior3, tdt_graf, ncol=2,
             botton = textGrob("Gráfico ?: Posteriors para tres datos",
                            x = 1,
                            just = "center"))
```
