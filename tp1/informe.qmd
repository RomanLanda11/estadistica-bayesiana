...
![Adultos mayores usufructuando las diferentes estrategias](img/adultos_jugando.jpg)
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, fig.height=3, fig.width=6  }
...
kable(summary(datos), col.names = c("Medidas resumen")) %>% 
  kable_styling(position = "center",full_width = FALSE) %>% 
  row_spec(0, bold = T, background = "gray", color = "black") 


ganancias0_df <- data.frame(anios=1:n_simulaciones, ganancias=resultados)
r0<-ggplot(ganancias0_df) +
      aes(x = ganancias) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)+
      labs(x = "Ganancias anuales",
         y = "") +
      theme_minimal() 
r0
```
...
grid.arrange(graf_list[[1]],graf_list[[3]],graf_list[[18]],
             graf_list[[100]],graf_list[[200]],graf_list[[366]],
             ncol = 3)
...
ganancias1_df <- data.frame(anios=1:n_simulaciones, ganancias=wins1_anio)
r1<-ggplot(ganancias1_df) +
      aes(x = ganancias) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)+
     labs(x="Ganancias anuales", y="")
r1
print(paste("La ganancia media por año es ",mean(ganancias1_df$ganancias),"unidades monetarias"))
...
ganancias2_df <- data.frame(anios=1:n_simulaciones, ganancias=wins2_anio)
r2<-ggplot(ganancias2_df) +
      aes(x = ganancias) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1) +
     labs(x="Ganancias anuales", y="")
r2
print(paste("La ganancia media por año es ",mean(ganancias2_df$ganancias),"unidades monetarias"))

```

Se grafica la distribución aproximada de la ganancia anual. En este se diferencian tres modas, estas corresponden a la cantidad de años en los cuales se usaron mayormente las máquinas uno, tres y dos respectivamente.
La media de las ganancias anuales es 185.8 unidades monetarias.
...
Viendo las distribuciones a posteriori se puede notar que los valores de la máquina dos está más concentrada alrededor de 0.55. Mientras que para las máquinas uno y tres la dispersión de los valores es mayor, esto se debe a que estas máquinas fueron las menos utilizadas.
...
ganancia3_df <- data.frame(wins = wins3_anio)
r3<-ggplot(ganancia3_df) +
      aes(x = wins) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)+
  labs(x="Ganancias anuales",y="")
r3
print(paste("La ganancia media por año es ",mean(ganancia3_df$wins),"unidades monetarias"))

```


Luego de simular 1000 años, se grafican las ganancias anuales y se observa una distribución bimodal con modas aproximadamente en 160 y 190 unidades monetarias, correspondientes a las maquinas dos y tres respectivamente. La ganancia media para los 1000 años de simulación resulta igual a 189.17 unidades monetarias.
El método es bayesiano ya que utiliza la máquina con probabilidad de éxito promedio más alta, es decir, la que presenta mayor esperanza en su distribución a posteriori y estas se actualizan en base a los resultados obtenidos tirada tras tirada
...
for (e in c(0.05, 0.2, 0.4)){
  ...
  ganancia4_df <- data.frame(wins = wins4_anio)
  graf_list[[j]] <-ggplot(ganancia4_df) + #Reemplazar por boxplots
        aes(x = wins) +
        geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
        geom_density(color = "blue", size = 1)+
        labs(x="Ganancias anuales",y="")

  print(paste("La ganancia media por año utilizando e = ",e,"  es",mean(mean(ganancia4_df$wins)),"unidades monetarias"))
  ganancias4_list[[j]] = ganancia4_df 
  j=j+1
}

grid.arrange(graf_list[[1]],graf_list[[2]],graf_list[[3]],ncol=1)
...
El método no es bayesiano ya que utiliza la máquina con tasa de éxito mayor con probabilidad $1-\epsilon$, y elige una máquina de manera aleatoria con probabilidad $\epsilon$, sin tener en cuenta las distribuciones a posteriori de cada máquina.
...
#Gráficos
graficar(cont_tiradas, cont_exitos, wins, alphas, betas)
print(paste("Las ganancias totales en ese año resultan ",wins[366], " unidades monetaris"))
```

Se observa que en el año simulado se utilizaron mayormente las máquinas tres y dos, es decir, las máquinas que presentan una mayor probabilidad de éxito.
Por este motivo, vemos que los gráficos a posteriori que más se modificaron en comparación al prior son los de estas 2 máquinas, dichas distribuciones parecen concentrarse en su verdadero valor poblacional a medida que se juega una mayor cantidad de veces.
Mientras que el último gráfico evidencia la evolución diaria de las ganancias a lo largo del año, obteniendo una ganancia total igual a 194 unidades monetarias.
...
ganancia5_df <- data.frame(wins = wins5_anio)
r5<-ggplot(ganancia5_df) +
      aes(x = wins) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)+
      labs(x="Ganancias anuales", y="")
r5
print(paste("La ganancia media por año es", mean(ganancia5_df$wins),"unidades monetarias"))

```

A traves de la simulación de los 1000 años, se observa la distribución de las ganancias, la cual presenta un comportamiento levemente asimétrico por izquierda donde el 95% de los valores son mayores a 157 unidades monetarias.
La ganancia media anual resulta igual a 185 unidades monetarias.
...
#Graficos
graficar(cont_tiradas, cont_exitos, wins, alphas, betas)
print(paste("La ganancia anual es igual a",wins[366],"unidades monetarias"))
```

Durante el año simulado se utilizó en mayor medida la máquina dos, la cual presenta una mayor probabilidad de éxito en una tirada.
El gráfico a posteriori de las máquinas uno y tres se ven levemente modificadas, mientras que dicha distribución para la máquina dos presenta valores mucho más concentrados alrededor del verdadero valor del parámetro.
Por último se registra la evolución diaria de las ganancias, obteniendo un total de 206 unidades monetarias.
...
ganancia6_df <- data.frame(wins = wins6_anio)
r6<-ggplot(ganancia6_df) +
      aes(x = wins) +
      geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
      geom_density(color = "blue", size = 1)+
      labs(x="Ganancias anuales", y="")
r6
print(paste("La ganancia media por año es", mean(ganancia6_df$wins),"unidades monetarias"))
```

Mediante el gráfico de las ganancias anuales se observa una distribución unimodal, con una leve asimetría a la izquierda en los cuales la ganancia anual se encuentra entre 166 y 211 unidades monetarias un 90% de las veces. La ganancia anual mediana resulta igual a 193 unidades monetarias.
...
# Graficar
graficar(cont_tiradas, cont_exitos, wins, alphas, betas)
print(paste("Las ganancias totales en ese año resultan ",wins[366], " unidades monetarias"))
```

Se observa que a lo largo del año se utilizó mayormente las máquinas dos y tres.
Las distribuciones a posteriori de las probabilidades de éxitos para estas máquinas se encuentran más concentradas en los valores reales de los parámetros, mientras que la distribución de la máquina 1 no difiere tanto de la distribución a priori, esto se debe a que fue la máquina menos utilizada.
El tercer gráfico presenta la evolución de las ganancias con el pasar de los días, se puede observar que la ganancia total resulta igual a 166 unidades monetarias.
...
ganancia7_df <- data.frame(wins = wins7_anio)
r7<- ggplot(ganancia7_df) +
        aes(x = wins) +
        geom_histogram(aes(y = ..density..), color = "black", fill = "lightblue", bins = 30) +
        geom_density(color = "blue", size = 1)+
      labs(x="Ganancias anuales", y="")
r7
print(paste("La ganancia media por año es", mean(ganancia7_df$wins),"unidades monetarias"))
```
...
La media de las ganancias anuales es 188.52 unidades monetarias.
...
Una vez simuladas todas las técnicas se procede a comparar los resultados obtenidos, y así poder recomendar a los futuros jugadores cuál de estas utilizar para maximizar las ganancias. Vale aclarar que se busca la estrategia que más se asemeje al caso ideal presentado al inicio del informe.
Se presenta una tabla comparando algunas estadísticas

```{r, echo=FALSE}
df_ganancias <- data.frame(Estrategia = rep(c("0. Caso ideal", "1. Completamente al azar", "2. Greedy con tasa observada", "3. Greedy con probabilidad a posteriori", "4. e-Greedy", "5. Softmax", "6. Upper-bound", "7. Thompson Sampling"),each=1000), Ganancia = c(ganancias0_df$ganancias, ganancias1_df$ganancias, ganancias2_df$ganancias, ganancia3_df$wins, ganancia4_df$wins, ganancia5_df$wins, ganancia6_df$wins, ganancia7_df$wins))

media_estrategia <- aggregate(Ganancia ~ Estrategia, df_ganancias, FUN= function(x) round(mean(x), 2))
desvio_estrategia <- aggregate(Ganancia ~ Estrategia, df_ganancias, FUN= function(x) round(sd(x), 2))
mediana_estrategia <- aggregate(Ganancia ~ Estrategia, df_ganancias, FUN= median)
q_estrategia <- aggregate(Ganancia ~ Estrategia, df_ganancias, FUN= function(x) round(quantile(x, 0.95), 2))

tabla_combinada <- merge(media_estrategia, desvio_estrategia, by = "Estrategia", suffixes = c(" media", " desvio"))
tabla_combinada <- merge(tabla_combinada, mediana_estrategia, by = "Estrategia")
tabla_combinada <- merge(tabla_combinada, q_estrategia, by = "Estrategia")


# Renombrar columnas
colnames(tabla_combinada) <- c("Estrategia", "Media", "Desvio", "Mediana", "Quantil 95")

kable(tabla_combinada)%>% 
  kable_styling(position = "center",full_width = FALSE) %>% 
  row_spec(0, bold = T, background = "gray", color = "black") 

```
Notoramiente las estrategias *completamente al azar* y $\epsilon-$*Greedy* presentan peores desempeños que el resto. Además, las medianas y cuantiles del resto de las estrategias se asemejan al caso ideal.
Para tomar una decisión final, se acude a la comparación de distribuciones de ganancias anuales aproximadas que se presentaron a lo largo del informe.


```{r, echo=FALSE, fig.height=6}
r0<- r0 + labs(title = "Caso ideal", size = 8)
r2<- r2 + labs(title = "Greedy con tasa obs.", size = 8)
r3<- r3 + labs(title = "Greedy con prob. post.", size = 8)
r5<- r5 + labs(title = "Softmax", size = 8)
r6<- r6 + labs(title = "Upper-bound", size = 8)
r7<- r7 + labs(title = "Thompson Sampling", size = 8)
grid.arrange(r0,r2,r3,
             r5,r6,r7,  ncol = 2)
```
