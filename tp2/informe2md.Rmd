...
Para entrar en calor, se comineza declarando una función que implemente el algoritmo de Metropolis-Hastings para tomar muestras de una distribución de probabilidad unidimensional. El algoritmo perimete elegir entre un punto de inicio arbitrario o al azar y utiliza la distribucion de propuesta de transición arbitraria (por defecto, se utiliza una distribución normal estándar).
...
sample_mh <- function (d_objetivo, r_propuesta, d_propuesta = d_propuesta_def, p_inicial, n){
  ...
  tasa_acepta = salto/n # TODO: agregar tasa de aceptación a la funcion
  return ( muestras ) 
}
```
...
Se observan las cadenas obtenidas utilizando tres grados de concentración distintos $kappas = 1,25, 50$
```{r, echo=FALSE}
kappas <- c(1,25,50)
n=5000
...
```
Se observa aleatoridad o un patron de *ruido blanco*.
Tambien, se presentan histogramas de frecuencias observadas para las distintas muestras.
...
```{r}
# TODO: averiguar si hay grafico para el correlograma
# TODO: agregar tasa de aceptación a la conclucion
...
df$posicion <- rep(seq(0, 1, length.out= 5000), time = 3)
ggplot(df, aes(x = V1, y = V2, color = cadena, alpha = posicion)) +
  geom_point() +
  geom_line() +
...
tp2=ggplot(df2, aes(x=x)) +
  ...
  geom_line(aes(y = V2), color = "#62A07B") +
  geom_line(aes(y = V3), color = "#613860") +
  labs(x = "Numero de muestra", y = "Valor", title = "")
...
grafico_corr_x1= ggplot(corr_x1)+
  ...
  geom_line()+
  labs(x = "", y = "Autocorrelación", title = "")+
  scale_color_manual(values= c("#536C8D","#62A07B","#613860"))
