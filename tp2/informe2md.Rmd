...
\begin{center}
  ...
  \includegraphics[width=0.4\textwidth]{img/logo_universidad}
...
graf3d <- persp(x, y, z, theta = 30, phi = 20, expand = 0.5, col = "#8b7991",
      xlab = "X", ylab = "Y", zlab = "Z", ticktype = "detailed", sub = "Función normal bivariada")
...
# Graficar
persp(x, y, z, theta = 60, phi = 30, expand = 0.5, col = "#8b7991",
...
b1 <- ggplot(muestras_2d1_df, aes(x = V1, y = V2)) +
  geom_density_2d(fill = "skyblue", alpha = 0.7) +
  geom_density2d_filled()+
  stat_contour(aes(x = x, y = Var2, z = z), data = df_grilla, color = "white", alpha = 0.3)+
  labs(title = "",
       x = "X1",
       y = "X2") +
  theme_minimal()
...
$$\Sigma_2=\begin{pmatrix}
   ...
   0 & 1
\end{pmatrix} $$
Las muestras del segundo *posterior* obtenidas resultan de la forma
```{r, echo=FALSE, out.width="90%"}
b2
```
...
$$\Sigma_3=\begin{pmatrix}
   ...
   -2 & 5
\end{pmatrix} $$
Las muestras del tercer *posterior* obtenidas resultan de la forma
```{r, echo=FALSE, out.width="90%"}
b3
```
